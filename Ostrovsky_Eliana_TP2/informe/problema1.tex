...
\begin{enumerate}
    ...
    \item \textbf{Detección y tratamiento de outliers}: Se implementó una técnica de dos pasos: primero, un algoritmo K-means (k=2) agrupó los valores de cada característica para identificar diferentes poblaciones celulares; luego, dentro de cada cluster se aplicó el método del rango intercuartílico (IQR), marcando como outliers los puntos que caían fuera de $Q1 - 1.5 \times IQR$ o $Q3 + 1.5 \times IQR$. Este enfoque permitió preservar las diferencias naturales entre poblaciones celulares mientras se identificaban valores extremos dentro de cada grupo.
    
    \item \textbf{Imputación de valores faltantes}: Los valores NaN resultantes de pasos anteriores y los originalmente faltantes fueron imputados mediante el algoritmo de k vecinos más cercanos (k-NN) con k=5. Para cada valor faltante, se identificaron los 5 registros más similares considerando las demás características disponibles, y se imputó la media de los valores correspondientes. Este método preserva mejor las relaciones entre variables que la imputación por media o mediana global.
...
El conjunto de datos contiene características morfológicas, bioquímicas y genéticas de células provenientes de muestras de tejido mamario. El análisis exploratorio reveló que algunas características como el tamaño celular, la tasa de mitosis y la densidad nuclear mostraban distribuciones bimodales, lo que sugiere la presencia de heterogeneidad en la población celular estudiada.

Las correlaciones entre variables mostraron asociaciones interesantes: se observó una correlación positiva entre la tasa de mitosis y el tamaño celular, mientras que la adhesión celular mostró correlación negativa con estos parámetros. Estos patrones de correlación ayudaron a guiar el preprocesamiento de datos y la posterior interpretación de los resultados del modelo.
...
A pesar de los prometedores resultados, el trabajo presenta algunas limitaciones que podrían abordarse en investigaciones futuras:

\begin{itemize}
    \item \textbf{Limitación en la captura de heterogeneidad tumoral}: El modelo actual no contempla la variabilidad intramuestra que puede existir en tumores heterogéneos. Una mejora sería incorporar características que capten esta heterogeneidad o implementar modelos jerárquicos que consideren múltiples regiones de la misma muestra.
    
    \item \textbf{Información contextual limitada}: No se incluyeron datos clínicos como edad, antecedentes familiares o información de imagen radiológica, que podrían enriquecer el diagnóstico. Integrar esta información en un modelo multimodal podría mejorar la precisión diagnóstica.
    
    \item \textbf{Falta de validación externa}: El modelo fue validado en conjuntos procedentes de la misma distribución estadística. La validación en datos de diferentes centros o con equipos de medición distintos sería necesaria para asegurar su generalización.
    
    \item \textbf{Binariedad del diagnóstico}: El enfoque actual se limita a clasificación binaria, mientras que en la práctica clínica existen categorías intermedias (como hiperplasia atípica o carcinoma in situ). Un modelo futuro podría incorporar clasificación multiclase para estos estados intermedios.
\end{itemize}

Como líneas de trabajo futuro, proponemos:

\begin{enumerate}
    \item Explorar arquitecturas más complejas como ensambles o modelos profundos que puedan capturar relaciones no lineales entre características.
    \item Desarrollar un sistema de explicabilidad que genere justificaciones automáticas de las predicciones, señalando qué características específicas llevaron a un diagnóstico particular.
    \item Implementar un sistema de aprendizaje continuo que pueda actualizarse con nuevos datos validados por patólogos, mejorando progresivamente su precisión.
    \item Extender el modelo para la predicción de subtipo molecular y agresividad tumoral, aspectos cruciales para el pronóstico y selección de tratamiento.
\end{enumerate}